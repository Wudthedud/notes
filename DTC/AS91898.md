...
4. Computational Overhead: Compared to parity bits, check digits use much more computational power. This is because for parity bits, all it needs to do is sum all the 1’s in the byte, whereas for check digits it has to perform an arithmetic operation. This means that there has to be much more processing.
### 4. Checksums
> [!NOTE]
...
- If an error occurs in any of the digits, the calculated checksum will not match the received checksum. A mismatch between the checksums indicates an error in one or more of the digits. 
- This method is efficient for catching single-digit errors and some types of transposition errors, enhancing data integrity.
...
- This can be done by adding all the bytes or words of the data block which produces a single-byte or two-byte result. For example, summing all bytes of the data and taking the modulo-256 of the sum.
	- Advantages of this method is that it's easy and quick to compute, with a low computational overhead. 
...
- Used in basic file integrity checks, simple data transmission protocols (TCP), low-security environments.
...
- A codeword is a sequence of symbols (or bits) that represents a piece of encoded information in an error-correcting code

They are defined by a set of parameters, usually denoted as (n, k) where:
- n: total number of symbols in the codeword
- k: number of symbols in the original data
- n−k: number of error correction symbols

###### **Examples of Reed-Solomon Codes:**
1. Compact Discs (CDs) and Digital Versatile Discs (DVDs)
When data is written to a CD or DVD, it is organized into sectors, which contains a certain amount of data. Each sector has Reed-Solomon error correction codes added to it to ensure that the data can be accurately read back even if some of it is damaged by scratches, dust, or other physical damage to the disc surface. 

Data on a CD is stored in blocks called frames. Each frame consists of 24 bytes of user data and 8 bytes of parity data generated by a Reed-Solomon encoder. CDs use a variant called Cross-Interleaved Reed-Solomon Code (CIRC). This involves two interleaved levels of Reed-Solomon error correction codes:
	1. CIRC uses two Reed-Solomon codes: one for handling smaller, random errors (inner code) and one for larger, burst errors (outer code) allowing Reed-Solomon codes to detect and correct the errors up to 4000 consecutive bits, which is crucial for handling scratches or fingerprints on the disc surface.
	2. Interleaving rearranges the data sequence before it is written to the CD. This ensures that consecutive data bits are spread out across different locations on the disc. If a scratch or defect damages a section of the disc, it affects only small parts of multiple codewords rather than an entire codeword, making it easier for the error correction system to fix the errors.


DVDs have higher storage capacity than CDs and use an enhanced version of Reed-Solomon codes called RS-PC (Reed-Solomon Product Code) which provides even greater error correction capabilities. This is necessary because DVDs have a higher data density than CDs, making them more susceptible to errors. Data is organized in sectors of 2,048 bytes, with additional bytes for error correction. 
DVDs use a two-dimensional Reed-Solomon error correction scheme:
- Codewords are arranged in rows and columns (a product code structure). Each row and column contains data along with error correction symbols, forming a grid.
- This two-dimensional structure allows the DVD player to correct errors both horizontally (row-wise) and vertically (column-wise), providing enhanced error recovery.
